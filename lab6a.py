...
class Student:
    ...
    def displayGPA(self):
        if len(self.courses) == 0: # Check if no courses are present
            return 'GPA of student ' + self.name + ' is ' + '0.0'
        
        gpa = 0.0 
        count_not_zero = 0 # Initialize counter for non-zero grades
        for course in self.courses.keys():
            gpa = gpa + self.courses[course] #add the grade 
            if self.courses[course] != 0.0:
                count_not_zero = count_not_zero + 1 # Increment counter for non-zero grades
        if count_not_zero == 0: 
            return 'GPA of student ' + self.name + ' is ' + '0.0'
    ...
    def displayCourses(self):
        ...
        for course in self.courses:
            if self.courses[course] > 0.0:# # Add the course to the passed courses list if the grade is greater than 0.0
